defmodule FracomexSynchroWeb.UsersController do
  use FracomexSynchroWeb, :controller
  alias FracomexSynchroWeb.Router.Helpers, as: Routes
  alias FracomexSynchro.Accounts
  alias FracomexSynchro.Accounts.User
  alias FracomexSynchro.Token
  alias FracomexSynchro.UserEmail
  alias FracomexSynchro.Products
  ...
  #   render(conn, "signin.html", layout: {FracomexSynchroWeb.LayoutView, "layout.html"})
  # end
  ...
  def submit_signup(conn, %{"user" => user_params}) do
    ...
    case Accounts.create_user(user_params) do
      {:ok, user} ->
        ...
        check_mail_url = "#{FracomexSynchroWeb.Endpoint.url()}#{Routes.users_path(conn, :check_signup_mail, token: token)}"
  ...
  def check_signup_mail(conn, %{"token" => token}) do
    ...
    with  {:ok, user_id} <- FracomexSynchro.Token.verify_new_account_token(token) do
      # IO.inspect(user_id)
  ...
  def submit_forgotten_password(conn, %{"user" => user_params}) do
    ...
    case Accounts.forgot_password(user_params) do
      ...
      _ ->
        ...
        token = Token.generate_new_account_token(user.id)
        forgot_password_mail_url = "#{FracomexSynchroWeb.Endpoint.url()}#{Routes.users_path(conn, :check_forgotten_password_mail, token: token)}"
