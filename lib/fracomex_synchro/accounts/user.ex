defmodule FracomexSynchro.Accounts.User do
  use Ecto.Schema
  ...
  defp check_if_account_is_not_already_valid(changeset) do
    ...
    list_mail_addresses = FracomexSynchro.Repo.all(FracomexSynchro.Accounts.User)
                          |> Enum.map(fn user ->
                            user.mail_address
                          end)
    cond do
      ...
      mail_address not in list_mail_addresses ->
       ...
       FracomexSynchro.Accounts.get_user_by_mail_address!(mail_address).is_valid ->
  ...
  defp check_if_account_is_valid(changeset) do
    ...
    cond do
      ...
      mail_address not in list_mail_addresses ->
        add_error(changeset, :mail_address, "Vous n'êtes pas encore inscrit")
      not FracomexSynchro.Accounts.get_user_by_mail_address!(mail_address).is_valid ->
  ...
  defp check_password(changeset) do
    ...
    cond do
      ...
      not Pbkdf2.verify_pass(password, FracomexSynchro.Accounts.get_user_by_mail_address!(mail_address).password) ->
