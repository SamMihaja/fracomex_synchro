defmodule FracomexSynchro.SyncLogic do
  alias FracomexSynchro.{EbpRepo, Products}
  alias FracomexSynchro.Products.{Family, SubFamily, Item}
  import Mogrify
  ...
  def update_items_diff_with_image() do
    Enum.each(select_valid_changesets_with_images(), fn changeset ->
      FracomexSynchro.Repo.update(changeset)
    end)
  end
  ...
  def insert_missing_item_families() do
    ...
    |> Enum.each(fn family -> FracomexSynchro.Repo.insert(family) end)
  end
